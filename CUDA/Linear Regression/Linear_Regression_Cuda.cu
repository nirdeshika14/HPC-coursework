...
double residual_err(double a, double y, double m, double c) {
  double e = (m * a) + c - y;
  return e * e;
}

__device__ double d_residual_err(double a, double y, double m, double c) {
  double e = (m * a) + c - y;
  return e * e;
}
...
double rms_error(double m, double c) {
  ...
  for(i=0; i<n_data; i++) {
    error_sum += residual_err(data[i].a, data[i].y, m, c);
  }
...
__global__ void d_rms_error(double *m, double *c, double *error_sum_arr, point_t *d_data) {
  ...
  error_sum_arr[i] = d_residual_err(d_data[i].a, d_data[i].y, *m, *c);
}
