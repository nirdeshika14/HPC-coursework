...
/******************************************************************************
  ...
  Compile with:
    gcc -o Posix1a Posix1a.c -lcrypt
...
void pwdcrack(char *salt_and_encrypted){
  ...
  for(x='A'; x<='Z'; x++){  //iterates over first intiital 
    for(y='A'; y<='Z'; y++){ //iterates over second initial 
      for(z=0; z<=99; z++){   //iterates over last two initials 00 to 99
