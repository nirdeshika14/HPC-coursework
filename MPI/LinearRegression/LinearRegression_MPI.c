 ...
 * 
 * To compile:
 *   mpicc -o LinearRegression_MPI LinearRegression_MPI.c -lm
 * 
 * To run:
 *   mpirun -n 9 ./LinearRegression_MPI
...
double residual_err(double a, double b, double m, double c) {
  double e = (m * a) + c - b;
  return e * e;
}
...
double rms_error(double m, double c) {
  ...
  double error_sum = 0;
  for(i=0; i<n_data; i++) {
    error_sum += residual_err(data[i].a, data[i].b, m, c);  
  }  
  mean = error_sum / n_data;  
  return sqrt(mean);
}

int time_diff(struct timespec *start, struct timespec *finish, long long int *difference) {
